...
plotTemperature <- function(Temp, lon, lat, discharge, 
                            fileName = "averageWaterTemperature_annual.png", 
                            legendTitle = "Water temperature (Celcius)", 
                            plotHeight = 1500, 
                            plotWidth=2500, 
                            pointsize = 36, 
                            Qlim = 50, 
                            ...
                            PlotBreaks = 5, 
                            PlotFactor = 0.035, 
                            cols=colorRampPalette(rev(c('#a50026','#d73027','#f46d43','#fdae61','#fee090','#ffffbf','#e0f3f8','#abd9e9','#74add1','#4575b4','#313695'))), 
                            sides=1, unit="C", ylim=c(-60,90), xlim=c(-168.5,191.5), size=1.3){
...
plotTemperature(Temp = as.matrix(td),lon = seq(-180+1/24,180,1/12), lat = seq(-60+1/24,90,1/12),discharge = as.matrix(q),
